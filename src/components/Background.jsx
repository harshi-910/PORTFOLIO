  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      blobRefs.current.forEach((blob, index) => {
        if (!blob) return; // Skip null blobs

        const initialPos = initialPositions[index];

        // Calculate movement in both X and Y direction
        const xOffset = Math.sin(currentScroll / 100 + index * 0.5) * 340; // Horizontal
        const yOffset = Math.cos(currentScroll / 100 + index * 0.5) * 40; // Vertical
